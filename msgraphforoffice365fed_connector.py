# File: msgraphforoffice365fed_connector.py
#
...
from msgraphforoffice365fed_consts import (
    MSGRAPHFOROFFICE365FED_CONNECTIVITY_ENDPOINT,
    MSGRAPHFOROFFICE365FED_ERR_CONNECTIVITY_TEST,
    MSGRAPHFOROFFICE365FED_SUCC_CONNECTIVITY_TEST,
)


class Msgraphforoffice365fedConnector(BaseConnector):
    """
    Msgraphforoffice365fed connector class that serves as a starting point for new connectors.
    """
    ...
    def _handle_test_connectivity(self, param):
        ...
        endpoint = MSGRAPHFOROFFICE365FED_CONNECTIVITY_ENDPOINT
        headers = {"Authorization": f"Bearer {self._api_key}"}

        ret_val, _ = self._make_rest_call(endpoint, action_result, headers=headers)

        if phantom.is_fail(ret_val):
            self.save_progress(MSGRAPHFOROFFICE365FED_ERR_CONNECTIVITY_TEST)
            return action_result.get_status()

        self.save_progress(MSGRAPHFOROFFICE365FED_SUCC_CONNECTIVITY_TEST)
...
if __name__ == "__main__":
    ...
    connector = Msgraphforoffice365fedConnector()
